<?php 
    ...
    if(empty($tmpfirstname)) {array_push($message, "First name is required!");}
    if(empty($tmplastname)) {array_push($message, "Last name is required!");}
    if(empty($tmpemail)) {array_push($message, "Email is required!");}
    ...
    if($db_output) {
        if($db_output['username'] === $tmpusername){array_push($message, "Username already exists!");}
        if($db_output['email'] === $tmpemail){array_push($message, "E-Mail already registered!");}
    }
